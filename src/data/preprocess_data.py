...
class Raw2Triples:
    ...
    def _str2ids(self, e1: str, r: str, e2: str, rr=None):
        ...
        r = dict_r[r] if r in dict_r.keys() else self._new_r(r)
        e2 = dict_e[e2] if e2 in dict_e.keys() else self._new_e(e2)
        if rr is not None: rr = dict_r[rr] if rr in dict_r.keys() else self._new_r(rr)
        return e1, r, e2, rr
    ...
    def _process1(self, line):
        ...
        if rr is not None: self.list_3.append([e2, rr, e1])  # save
    ...
    def _process(self, path):
        with open(path) as f:
            for line in tqdm(f, total=self._file_length(path), leave=False):
                self._process1(line)
    ...
    def process(self, *, logger):
        for mode, _file in (
            ...
            with h5py.File(save_hdf_path, mode='a') as f:
                f.create_dataset('triple', data=np.array(self.list_3))
...
class Triple2StudyCase:
    ...
    def _add_er(self, e1, r, e2, data_type):
        key2id = self.er2id
        key, value = (e1, r), e2
        id_ = self._new_id(key) if key not in key2id.keys() else key2id[key]
